...
2. WHEN 页面加载完成 THEN "颜色配置"区域的标题栏右侧 SHALL 展示种子颜色色块和"智能配色"按钮，布局紧凑，与标题在同一行。
3. WHEN 用户点击种子颜色色块 THEN 系统 SHALL 展开/收起颜色选择器（HexColorPicker），选择器显示在标题栏下方。
4. WHEN 用户点击"智能配色"按钮 THEN 系统 SHALL 基于当前种子颜色生成 M3 配色并应用到 4 个颜色通道。
5. WHEN 种子颜色变化时 THEN 系统 SHALL 保持现有的防抖逻辑（150ms 延迟智能配色）。
